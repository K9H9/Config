...
local function emit_volume_info()
    awful.spawn.with_shell("pamixer --get-volume", function(vol)
        local volume_int = tonumber(vol)
    end)

    awful.spawn.with_shell("pamixer --get-mute", function(mute_stat)
        if mute_stat  == "false"
            local mute = 0
            local muted_int = mute and 1 or 0 
        else
            local mute = 1
            local muted_int = mute and 1 or 0
    ...
    --     "pactl list sinks | awk '/\\* index: /{nr[NR+7];nr[NR+11]}; NR in nr'",
    --     function(stdout)
